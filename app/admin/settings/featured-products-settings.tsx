...
interface FeaturedProductsSettingsProps {
  ...
  onFeaturedProductsChange: (products: string[]) => void
}

export default function FeaturedProductsSettings({ 
  featuredProducts, 
  onFeaturedProductsChange 
}: FeaturedProductsSettingsProps) {
          ...
          if (query.trim().length >= 2) {
            ...
            try {
              ...
              const filtered = result.products.filter((p: any) => 
                !featuredProducts.includes(p.id) && p.isPublished
              )
  ...
  const addProduct = (product: any) => {
    ...
    const newProducts = [...featuredProducts, product.id]
    onFeaturedProductsChange(newProducts)
    ...
    toast({
      title: 'Product Added',
      description: `${product.name} has been added to featured products`,
      variant: 'default'
    })
  }

  const removeProduct = (productId: string) => {
    const newProducts = featuredProducts.filter(id => id !== productId)
    onFeaturedProductsChange(newProducts)
    toast({
    ...
    <Card>
      <CardHeader>
        ...
        <p className='text-sm text-muted-foreground'>
          Select products to feature on the homepage. These will appear below the hero section.
        </p>
      </CardHeader>
      <CardContent className='space-y-6'>
        ...
        {/* Selected Products */}
        <div className='space-y-4'>
          <Label>Featured Products ({selectedProducts.length})</Label>
          {selectedProducts.length === 0 ? (
            <div className='text-center py-8 text-gray-400 border border-gray-700 rounded-lg'>
              <p className='text-white'>No featured products yet</p>
              <p className='text-sm text-gray-400'>Search and add products to feature them on the homepage</p>
            </div>
          ) : (
